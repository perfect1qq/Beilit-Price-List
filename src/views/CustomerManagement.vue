            ...
            <div class="card-footer">
              <div class="action-buttons">
                ...
                <template v-if="!isGuest">
                  <el-button v-if="canEdit" type="warning" size="small" plain @click.stop="handleEdit(item)">编辑</el-button>
                  <el-button v-if="canDelete" type="danger" size="small" plain @click.stop="handleDelete(item)">删除</el-button>
                </template>
              </div>
            </div>
          </div>
...
const handleGoToQuotation = (item) => {
  if (item.quotationId) {
    router.push({ path: '/quotation/history', query: { id: item.quotationId, mode: 'view' } })
  }
}
...
.search-filter-row .search-bar { flex: 1; min-width: 300px; }

.filter-group { display: flex; align-items: center; gap: 12px; flex-shrink: 0; }

.customer-card .info-row .remark-text { color: #909399; font-size: 13px; font-style: italic; }

.quotation-info { display: flex; align-items: center; gap: 8px; flex-wrap: wrap; }

.quotation-date { color: #67c23a; font-size: 13px; font-weight: 500; }
...
.follow-up-content { display: flex; flex-direction: column; gap: 4px; width: 100%; }
...
.follow-up-meta { display: flex; align-items: center; gap: 8px; }

.follow-up-time { color: #909399; font-size: 12px; }
</style>
