<!--
  ...
  页面布局：
  ┌──────────────────────────────────────────────────────────────┐
  │  UsdConversion (美金换算工具)                                 │
  │                                                              │
  │  Header: 美金换算工具 [+添加一行]                            │
  │                                                              │
  │  Global Settings (全局设置区)                                 │
  │  ┌────────────────────────────────────────────────────────┐  │
  │  │ 全局汇率: [7.2]                                        │  │
  │  │ 原单总人民币: ¥[10000]                                  │  │
  │  │ 国内运费: ¥[500] ➔ $[69.44]                           │  │
  ...
  │  │ 数量     │ 单价(¥)      │ 小计(¥)  │ 单价($)  │ 小计($ )│  │
  │  ├──────────┼──────────────┼──────────┼──────────┼────────┤  │
  │  │ [100]    │ [50.00]      │ 5000.00  │ 6.94     │ 694.44 │  │
  │  │ [200]    │ [30.00]      │ 6000.00  │ 4.17     │ 833.33 │  │
  │  │ ...      │ ...          │ ...      │ ...      │ ...    │  │
  │  ├──────────┼──────────────┼──────────┼──────────┼────────┤  │
  │  │ 合计     │              │ 11000.00 │          │1527.78 │  │
  │  └──────────┴──────────────┴──────────┴──────────┴────────┘  │
    ...
    <el-card class="box-card">
      ...
      <div class="global-setting-group">
        <div class="setting-row">
          ...
          <el-input v-model="globalExchangeRate" @input="handleRateChange" class="input-item w-120"
            input-style="text-align: center" />
        </div>

        <div class="setting-row">
          <label class="label-title">原单总人民币：</label>
          <el-input v-model="manualTotalRmbBase" class="input-item w-150" input-style="text-align: center">
        ...
        <div class="setting-row">
          ...
          <el-input v-model="globalDomesticRmb" @input="handleDomesticRmbChange" class="input-item w-150"
            input-style="text-align: center">
            <template #prefix>¥</template>
          </el-input>
          <span class="arrow">➔</span>
          <el-input v-model="globalDomesticUsd" @input="handleDomesticUsdChange" class="input-item w-150"
            input-style="text-align: center">
            <template #prefix>$</template>
          </el-input>
        </div>
        ...
        <div class="setting-row">
          ...
          <el-input v-model="globalIntlRmb" @input="handleIntlRmbChange" class="input-item w-150"
            input-style="text-align: center">
            <template #prefix>¥</template>
          </el-input>
          <span class="arrow">➔</span>
          <el-input v-model="globalIntlUsd" @input="handleIntlUsdChange" class="input-item w-150"
            input-style="text-align: center">
            <template #prefix>$</template>
          </el-input>
        </div>
      ...
      <div class="footer-summary">
        ...
        <div class="info-alert" :class="Number(diffRmb) >= 0 ? 'success-bg' : 'error-bg'">
          ...
          <div class="info-content">
            ...
            <span>
              当 <b>相差人民币</b> 为 <span class="usd-text font-bold">正数</span>
              时，说明您当前排版的美金报价按汇率折现回来后，<b>比原先人民币表单总计金额多</b>，您将获得溢价收入。
              <br />
              当 <b>相差人民币</b> 为 <span class="highlight font-bold">负数</span>
              时，说明换算回来的钱<b>比原本的人民币成本还要少</b>，存在报价亏损风险，请提高单价或汇率。
            </span>
          </div>
        </div>
      </div>
