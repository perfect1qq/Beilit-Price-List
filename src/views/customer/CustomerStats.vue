...
defineProps<{ stats: Record<string, number>, activeStat: string }>();
const emit = defineEmits<{ (e: "stat-click", stat: string): void }>();
const handleStatClick = (stat: string) => { emit("stat-click", stat); };
</script>
<style scoped>
.stats-row {
  display: flex;
